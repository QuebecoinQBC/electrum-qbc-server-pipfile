How to run your own Electrum Quebecoin server
========================================
...
Electrum Quebecoin server on Linux. It is structured as a series of steps you need to
...
document, we assume that user is called 'quebecoin'). We also assume the
quebecoin user has sudo rights, so we use `$ sudo command` when we need to.
...
you also need to run quebecoind and keep a copy of the full blockchain,
which is roughly 55 GB (February 2016). Ideally you have a machine with 16 GB of RAM
and an equal amount of swap. If you have ~2 GB of RAM make sure you limit quebecoind 
...
only leaving spare cycles for quebecoind. Fast single core CPU power helps for the initial 
...
### Step 1. Create a user for running quebecoind and Electrum Quebecoin server

This step is optional, but for better security and resource separation I
suggest you create a separate user just for running `quebecoind` and Electrum.
We will also use the `~/bin` directory to keep locally installed files
(others might want to use `/usr/local/bin` instead). We will download source
code files to the `~/src` directory.

    $ sudo adduser quebecoin --disabled-password
    $ sudo apt-get install git
    $ sudo su - quebecoin
    $ mkdir ~/bin ~/src
    $ echo $PATH

If you don't see `/home/quebecoin/bin` in the output, you should add this line
to your `.bashrc`, `.profile`, or `.bash_profile`, then logout and relogin:

    PATH="$HOME/bin:$PATH"
    $ exit

### Step 2. Download quebecoind

### Step 3. Configure and start quebecoind

In order to allow Electrum Quebecoin to "talk" to `quebecoind`, we need to set up an RPC
username and password for `quebecoind`. We will then start `quebecoind` and
wait for it to complete downloading the blockchain.

    $ mkdir ~/.quebecoin
    $ $EDITOR ~/.quebecoin/quebecoin.conf

Write this in `quebecoin.conf`:
...
If you have an existing installation of quebecoind and have not previously
set txindex=1 you need to reindex the blockchain by running

    $ quebecoind -reindex

If you already have a freshly indexed copy of the blockchain with txindex start `quebecoind`:

    $ quebecoind

Allow some time to pass for `quebecoind` to connect to the network and start
downloading blocks. You can check its progress by running:

    $ quebecoin-cli getblockchaininfo

Before starting the Electrum Quebecoin server your quebecoind should have processed all
blocks and caught up to the current height of the network (not just the headers).
You should also set up your system to automatically start quebecoind at boot
time, running as the 'quebecoin' user. Check your system documentation to
find out the best way to do this.

### Step 4. Download and install Electrum Quebecoin server

We will download the latest git snapshot for Electrum to configure and install it:

    $ cd ~
    $ git clone https://github.com/ghonyme/electrum-quebecoin-server.git
    $ cd electrum-quebecoin-server
...
### Optional Step 5: Install Electrum Quebecoin dependencies manually

Electrum Quebecoin server depends on various standard Python libraries and leveldb. These will usually be
...
Electrum Quebecoin server uses leveldb to store transactions. You can choose
...
and further instructions from the Electrum Quebecoin full archival server foundry at a later time.
...
server.key corresponds to `ssl_keyfile=` in your Electrum Quebecoin server config.
...
### Step 9. Configure Electrum Quebecoin server

Electrum Quebecoin reads a config file (/etc/electrum-quebecoin.conf) when starting up. This
file includes the database setup, quebecoind RPC setup, and a few other
options.

The "configure" script listed above will create a config file at /etc/electrum-quebecoin.conf
...
Electrum Quebecoin server currently needs quite a few file handles to use leveldb. It also requires
file handles for each connection made to the server. It's good practice to increase the
open files limit to 64k.

The "configure" script will take care of this and ask you to create a user for running electrum-quebecoin-server.
If you're using the user `quebecoin` to run electrum and have added it as shown in this document, run
the following code to add the limits to your /etc/security/limits.conf:

     echo "quebecoin hard nofile 65536" >> /etc/security/limits.conf
     echo "quebecoin soft nofile 65536" >> /etc/security/limits.conf
...
Check if the limits are changed either by logging with the user configured to run Electrum Quebecoin server as. Example:
...
Or if you use sudo and the user is added to sudoers group:
    ...
    sudo -u quebecoin -i ulimit -n


Two more things for you to consider:

1. To increase privacy of transactions going through your server
   you may want to close quebecoind for incoming connections and connect outbound only. Most servers do run
   full nodes with open incoming connections though.

2. Consider restarting quebecoind (together with electrum-server) on a weekly basis to clear out unconfirmed
   transactions from the local the memory pool which did not propagate over the network.

### Step 11. (Finally!) Run Electrum Quebecoin server

The magic moment has come: you can now start your Electrum Quebecoin server as root (it will su to your unprivileged user):
...
You should see this in the log file:
    ...
    starting Electrum Quebecoin server
...
If you want to stop Electrum Quebecoin server, use the 'stop' command:
...
### Step 12. Test the Electrum Quebecoin server
...
current server and connect to your new Electrum Quebecoin server. You should see your
addresses and transactions history. You can see the number of blocks and
response time in the server selection window. You should send/receive some
quebecoins to confirm that everything is working properly.
...
If you're operating a public Electrum Quebecoin server please subscribe
